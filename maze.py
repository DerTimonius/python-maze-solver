...
class Maze: 
    ...
    def _solve_r(self, i, j):
        ...
        if end_cell == curr_cell:
            print("found a way out")
            return True
        # go up
        if j -1 >= 0 and not curr_cell.has_top_wall and not self._cells[i][j-1].visited:
        ...
        if  j + 1 < self._num_rows and not curr_cell.has_bottom_wall and not self._cells[i][j + 1].visited:
            ...
            if self._solve_r(i, j +1):
                ...
                curr_cell.draw_move(self._cells[i][j+1], True)
        # go left
        if i - 1 >= 0 and not curr_cell.has_left_wall and not self._cells[i-1][j].visited:
